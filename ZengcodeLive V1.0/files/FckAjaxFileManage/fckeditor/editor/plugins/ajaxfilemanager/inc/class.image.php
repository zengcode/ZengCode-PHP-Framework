<?php
	...
	function resize( $new_x, $new_y, $constraint= false, $unchangeIfsmaller=false)
	{
		...
		}else 
		{
	      ...
	      if($new_x > 0 && $new_y > 0)
	      {
	      	$ratio = min($new_x/$this->_imgInfoOrig['width'], $new_y / $this->_imgInfoOrig['height']);
	      }elseif($new_x > 0)
	      {
	      	$ratio = $new_x / $this->_imgInfoOrig['width'];
	      }elseif($new_y > 0)
	      {
	      	$ratio = $new_y / $this->_imgInfoOrig['height'];
	      }else 
	      {
	      	$ratio = 1;
	      }
	      ...
	      $ratio = ($ratio > 1?1:$ratio);	 		
		    $new_x = $ratio * $this->_imgInfoOrig['width'];
		    $new_y = $ratio * $this->_imgInfoOrig['height'];					
