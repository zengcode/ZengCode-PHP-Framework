...
class Upload
{
	...
	function resize($filePath, $thumb_suffix="", $new_x = 0, $new_y = 0)
	{
		...
		if(empty($filePath))
		{
	      ...
	      if($new_x > 0 && $new_y > 0)
	      {
	      	$ratio = min($new_x/$this->img_x, $new_y / $this->img_y);
	      }elseif($new_x > 0)
	      {
	      	$ratio = $new_x / $this->img_x;
	      }elseif($new_y > 0)
	      {
	      	$ratio = $new_y / $this->img_y;
	      }
	      $ratio = ($ratio > 1?1:$ratio);	 		
		    $new_x = $ratio * $this->img_x;
		    $new_y = $ratio * $this->img_y;
	 ...
	 */
	function getThumbnailDetails($originalImageName, $originaleImageWidth, $originalImageHeight, $thumbnailSuffix, $thumbnailWidth, $thumbnailHeight)
	{
		...
		if(!empty($originalImageName) && !empty($originaleImageWidth) && !empty($originalImageHeight))
		{
			...
			$new_x = $thumbnailWidth;
			$new_y = $thumbnailHeight;
	      if($new_x > 0 && $new_y > 0)
	      {
	      	$ratio = min($new_x/$originaleImageWidth, $new_y / $originalImageHeight);
	      }elseif($new_x > 0)
	      {
	      	$ratio = $new_x / $originaleImageWidth;
	      }elseif($new_y > 0)
	      {
	      	$ratio = $new_y / $originalImageHeight;
	      }else 
	      {
	      	$ratio = 1;
	      }
	      ...
	      $ratio = ($ratio > 1?1:$ratio);	 		
		    $new_x = $ratio * $originaleImageWidth;
		    $new_y = $ratio * $originalImageHeight;			

			$outputs['width'] = $new_x;
			$outputs['height'] = $new_y;
		}
		return $outputs;
