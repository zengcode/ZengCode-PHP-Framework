            ...
            if (jQuery.isFunction(settings.target)) {
				...
				var errorPrefix = 'error:';
					...
					}else
					{
						...
						var queryStrPrefix = 'querystr:';
						var fileNameIndex = str.indexOf(fileNamePrefix);
						var filePathIndex = str.indexOf(filePathPrefix);
						var queryStrIndex = str.indexOf(queryStrPrefix);
						
						if(fileNameIndex > -1 && filePathIndex > -1 && queryStrIndex > - 1)
						{
							var fileName = str.substring(fileNameIndex + fileNamePrefix.length, queryStrIndex);
							var filePath = str.substr(filePathPrefix.length, (fileNameIndex - fileNamePrefix.length));
							var queryStr = str.substr(queryStrIndex + queryStrPrefix.length);
							...
							switch($('#itemType'+rowNum).val())
							{
								...
								case "folder":
									...
									$(icon).attr('href', (iconUrl.substring(0, iconUrl.lastIndexOf('?path=') + 6) + filePath + queryStr));
